...
export const api = (db: IDatabase) => {
    ...
    router.get("/company-graph", async (req, res) => {
        ...
        for (let i = 0; i < ownerships.length; i += 1000) {
            const c: Company[] = await db.companies.find({ orgnr: { $in: ownerships.map(o => (o.shareholderOrgnr as string)).slice(i, i + 1000) } }).toArray()
            for (const company of c) {
                nodes[company.orgnr] = company;
            }
            const s: Shareholder[] = await db.shareholders.find({ orgnr: { $in: ownerships.map(o => (o.shareholderOrgnr as string)).slice(i, i + 1000) } }).toArray()
