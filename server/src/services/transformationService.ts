...
export const transformData = async (db: IDatabase, year: 2020 | 2019) => {
...
const writeInvestorCount = async (db: IDatabase, year: 2020 | 2019) => {
    console.log('writeInvestorCount started.');
    const investorCounts: { [orgnr: string]: number } = {};
    const ownerships = await db.ownerships.find({ year }).toArray();
...
const writeInvestmentCount = async (db: IDatabase, year: 2020 | 2019) => {
