...
import { GraphLinkType, GraphType, IndirectOwnership, UserEventType } from "../../models/models";
...
export const IndirectOwnershipTable = () => {
  ...
  // Show the investor's ownership chains as directed OWNS paths in the graph itself.
  const showChainsInGraph = (ownership: IndirectOwnership) => {
    if (!source?.properties.uuid) return;
    dispatch(resetGraph());
    dispatch(close());
    history.push(
      `?graphType=${GraphType.AllPaths}&sourceUuid=${ownership.investor.properties.uuid}&targetUuid=${source.properties.uuid}&isDirected=true&linkTypes=${GraphLinkType.OWNS}`
    );
  };
    ...
    return (
      <div className="flex flex-col justify-between w-full h-full overflow-auto mt-12">
        ...
        <p className="text-center text-xs pb-1" style={{ color: theme.muted }}>
          Effektiv eierandel er summen over alle eierskapskjeder av produktet av eierandelene i hvert ledd. Klikk på en
          eier for å se eierskapskjedene i grafen.
        </p>
          ...
          {owners.length === 0 ? (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left" style={{ color: theme.muted }}>
                  ...
                  <th
                    ...
                    title="Antall ulike eierskapskjeder fra eieren til selskapet. Klikk på en rad for å vise kjedene i grafen."
                  >
                    Kjeder
                  </th>
                </tr>
              ...
              <tbody>
                {owners.map((o) => (
                  <tr
                    key={o.investor.properties.uuid}
                    className="border-t border-primary/20 cursor-pointer"
                    onClick={() => showChainsInGraph(o)}
                    title="Vis eierskapskjedene i grafen"
                  >
                    <td className="p-2">
                      <p className="flex items-center">
                        <span className="mr-2 flex-shrink-0">
                          <GraphLogo width="14px" height="14px" />
                        </span>
